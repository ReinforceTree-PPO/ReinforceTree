...
def parse_results_dir(dir_path, inference_tasks):
    ...
    for task_id, task in enumerate(inference_tasks):
        for eval_id in range(task_id + 1):
            ...
            try:
                ...
                print(f"\tWarning: {current_file_name} not found, skipping...")
                continue
            except Exception as e:
                print(f"\tError processing {current_file_name}: {e}")
                continue
    ...
    if len(all_results) > 0:
        ...
        table_str += ("All Average: N/A (no results)") + '\n'
        table_str += ("Last Average: N/A (no results)") + '\n'
    ...
    try:
        with open(f"{dir_path}/final_result.txt", "w") as f:
            f.write(table_str)
        print(f"Results saved to {dir_path}/final_result.txt")
    except Exception as e:
