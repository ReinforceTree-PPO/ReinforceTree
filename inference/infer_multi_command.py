    ...
    for round in range(start_round, task_num):  # load models and adapters of a new round in continual learning
        ...
        for inference_task_id in range(round + 1):  # evaluation for previous tasks in a single round
            ...
            for rank, result in results:
                if result is None:
                    print(f"Warning: RANK {rank} returned None, skipping...")
                    continue
                if inference_task not in result:
                    print(f"Warning: Task {inference_task} not found in RANK {rank} results")
                    continue
            ...
            # Skip evaluation if no predictions were generated
            if len(predicted_sequences) == 0:
                print(f"Warning: No predictions for task {inference_task}, skipping evaluation...")
                continue
            ...
            try:
                ...
                print(f"Error evaluating task {inference_task}: {e}")
...
def run_inference(current_rank, args):
    ...
    while fail_num < 3:
        try:
            # only need last line:
            output_lines = os.popen(" ".join(command)).readlines()
            if len(output_lines) < 2:
                raise Exception("Insufficient output lines")
            _ = output_lines[-2]
            # read str in args.output_json_file_path:
            if not os.path.exists(args.output_json_file_path):
                raise FileNotFoundError(f"Output file not found: {args.output_json_file_path}")
